########################################################################################################################
# Script Name: Span and Style Attribute Remover For Canvas Pages - Single Page Version
#
# Developed by: Scott Beaulieu
#
# Contact Information: nicholassjbeaulieu@protonmail.com
#
# Description: Unwraps all span tags and removes all style attributes (in that order) for a single specified Canvas page
########################################################################################################################
...
def targetPageSelection():
    targetPage = (input("Please enter the title of the target page the formatting and span tags should be "
                        "removed from (this input is not case or spacing dependent): "))
    return targetPage
...
def getSpecificPage(pageTitle):
    ...
    for page in fullList:
        if (page['title'].lower()).replace(" ", "") == (pageTitle.lower()).replace(" ", ""):
            return page
    return
...
targetPageTitle = targetPageSelection()
targetPage = getSpecificPage(targetPageTitle)
retrievedPage = (requests.get(url + '/api/v1/courses/{}/pages/{}'.format(courseID, targetPage['url']),
